 ...
 * problem taken from:
 *
 * https://www.mathworks.com/help/optim/ug/nonlinear-inequality-constraints.html
 *
 * */
...
class Ex2Variables : public VariableSet
{
 public:
  ...
  Ex2Variables(const std::string& name)
      : VariableSet(2, name)
  {
...
class Ex2Constraint : public ConstraintSet
{
 public:
  Ex2Constraint()
  ...
  Ex2Constraint(const std::string& name)
  ...
  {
    VecBound b(GetRows());
    b.at(0) = Bounds(-inf, -1.5);
    b.at(1) = Bounds(-inf, 10.0);
    return b;
  }
  ...
  void FillJacobianBlock(std::string var_set, Jacobian& jac_block) const override
  {
    ...
    if (var_set == "var_set1") {
      ...
      jac_block.coeffRef(0, 0) = x(1) - 1;  // derivative of first constraint w.r.t x0
      jac_block.coeffRef(0, 1) = x(0) - 1;      // derivative of first constraint w.r.t x1
      jac_block.coeffRef(1, 0) = -x(1);  // derivative of first constraint w.r.t x0
      jac_block.coeffRef(1, 1) = -x(0);      // derivative of first constraint w.r.t x1
    }
  }
};

class Ex2Cost : public CostTerm
{
 public:
  Ex2Cost()
      : Ex2Cost("cost_term1")
  ...
  {
    Vector2d x = GetVariables()->GetComponent("var_set1")->GetValues();
    return std::exp(x(0)) * (4 * x(0) * x(0) + 2 * x(1) * x(1) + 4 * x(0) * x(1) + 2 * x(1) + 1);
  }
  ...
  void FillJacobianBlock(std::string var_set, Jacobian& jac) const override
  {
    if (var_set == "var_set1") {
      ...
      jac.coeffRef(0, 0) = std::exp(x(0)) * (4 * x(0) * x(0) + 2 * x(1) * x(1) + 4 * x(0) * (x(1)+2) + 6 * x(1) + 1);
      jac.coeffRef(0, 1) = std::exp(x(0)) * (4 * x(0) + 4 * x(1) + 2 );
    }
  }
};
